...
| Triệu chứng | Fix |
|---|---|
| `DatasetNotFoundError: Vietnamese-alpaca-cleaned` | Dataset removed from HF Hub — default is now `tsdocode/vi_alpaca_clean` (set `SFT_DATASET` in `.env` to override) |
| `chat_template is not set` on Colab | Re-run from section A (writes `colab_compat.py`) or `make colab-t4` to refresh `Lab22_DPO_T4.ipynb` |
| `memory_efficient_attention_backward` on T4 DPO | **Restart runtime**, re-run section A (uninstalls `xformers`, forces SDPA). If still failing, refresh notebook via `make colab-t4` |
...
| NB5 `reverse_op` / `Failed to save model` on T4 | Dùng `export_gguf_colab()` (NB5 §1) — tránh `save_pretrained_merged`. Hoặc pin `transformers<5`, hoặc skip NB5 |
...
- **Datasets:** UltraFeedback (Argilla), `tsdocode/vi_alpaca_clean` (VN Alpaca SFT; replaces removed `5CD-AI/Vietnamese-alpaca-cleaned`)
